...
class DataPreprocessingToolkit(object):
    ...
    def filter_out_company_clients(df):
        ...
        :rtype: pd.DataFrame
        """
        filtered_clients = df[df["is_company"] == 0]
    ...
    def filter_out_long_stays(df):
        ...
        :rtype: pd.DataFrame
        """
        filtered_stays = df[df["length_of_stay"] <= 21]
    ...
    def filter_out_low_prices(df):
        ...
        :rtype: pd.DataFrame
        """
        filtered_prices = df[df["accommodation_price"] > 50]
    ...
    def add_length_of_stay(df):
        ...
        :rtype: pd.DataFrame
        """
        length_of_stay = df['date_to'] - df['date_from']
        df['length_of_stay'] = length_of_stay.dt.days
        return df
    ...
    def add_book_to_arrival(df):
        ...
        :rtype: pd.DataFrame
        """
        book_to_arrival = df['date_from'] - df['booking_date']
        df['book_to_arrival'] = book_to_arrival.dt.days
        return df
    ...
    def add_weekend_stay(df):
        ...
        datediff = (df["date_to"] - df["date_from"]).dt.days
        df["weekend_stay"] = (((start >= 4) & (start != 6)) | (end >= 5) | ((end < start) & (start != 6)) | (datediff >= 6)).apply(str)
        return df
    ...
    def add_night_price(df):
        ...
        :rtype: pd.DataFrame
        """
        night_price = df['accommodation_price'] / df['length_of_stay'] / df['n_rooms']
        df['night_price'] = night_price.round(2)
        return df
    ...
    def aggregate_group_reservations(self, df):
        ...
        sum = group_reservations[["group_id"] + self.sum_columns].groupby(["group_id"]).sum()
        mean = group_reservations[["group_id"] + self.mean_columns].groupby(["group_id"]).mean()
        mode = group_reservations[["group_id"] + self.mode_columns].groupby(["group_id"])\
            .agg(lambda x: x.value_counts().index[0])
        first = group_reservations[["group_id"] + self.first_columns].groupby(["group_id"]).first()

        group_reservations = sum.merge(mean, on="group_id")\
            .merge(mode, on="group_id").merge(first, on="group_id")
        aggregated = pd.concat([non_group_reservations, group_reservations])
        return aggregated
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        :rtype: pd.DataFrame
        """
        average_night_prices = df.groupby("room_group_id")["night_price"].mean()
        average_night_prices = df.merge(average_night_prices, on="night_price", how="left")
        df['room_segment'] = average_night_prices["night_price"]\
            .apply(lambda x: self.map_value_to_bucket(x, self.room_segment_buckets))
        return df
